...
def format_time_diff(s, e):
    ...
    return "{0:02d}:{1:02d}:{2:02d}".format(
        int(hours), int(minutes), int(seconds + 0.5)
    )
...
def download_image(logger, url, save_to, stream=True, attempts=5, sleep=30, w=""):
    ...
    for attempt in range(1, attempts + 1):
        try:
            ...
            logger.warning(
                "  could not retrieve, now sleep for {0}s".format(sleep * attempt)
            )
        ...
        logger.info(
            "{0}  attempt {1}/{2} --> status_code {3}".format(
...
def call_download_image(logger, url, save_to, instance, field, ctpk, userpk, cmd_name):
    if not os.path.exists(save_to) or not os.path.isfile(save_to):
        ...
        setattr(
            ...
            field,
            "https://www.mancelot.app"
            + save_to.replace("{0}/".format(settings.STATIC_ROOT), settings.STATIC_URL),
        )
        LogEntry.objects.log_action(
            user_id=userpk,
            content_type_id=ctpk,
...
def generate_thumbnail(logger, img, size=(120, 120), w=""):
    if not os.path.exists(img) or not os.path.isfile(img):
        logger.warning(
            "{0}WARNING in generate_thumbnail: img '{1}' does not exist!".format(w, img)
        )
        return
    ...
    if not extension:  # TODO: fix
        logger.error(
            "{0}ERROR in generate_thumbnail: img '{1}' has no extension ".format(w, img)
        )
        return
...
def optimize_image(logger, img, w=""):
    if not os.path.exists(img) or not os.path.isfile(img):
        logger.warning(
            "{0}WARNING in optimize_image: img '{1}' does not exist!".format(w, img)
        )
        return
    ...
    if not extension:  # TODO: fix
        logger.error(
            "{0}ERROR in optimize_image: img '{1}' has no extension ".format(w, img)
        )
        return
    ...
    if extension == ".jpg" or extension == ".jpeg":
        ...
        commands.append([command] + options + [img])
    elif extension == ".png":
        command = "pngquant"
        options = [
    ...
    for command in commands:
        ...
        if err:
            logger.debug("{0}ERROR: {1}".format(w, err))
...
class CommandWrapper(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if is_task:
            self.logger.handlers = []  # do not log to console
        ...
        self.logger.info(
            "manage {0}: started, is_task={1}".format(self.cmd_name, is_task)
        )
        self.logger.info(
            "Start time: {0}\n".format(
                timezone.localtime(start_time).strftime("%d/%m/%Y %H:%M:%S")
            )
        )
        ...
        self.logger.info(
            "End time: {0}".format(
                timezone.localtime(end_time).strftime("%d/%m/%Y %H:%M:%S")
            )
        )
        self.logger.info(
            "Execution time: {0}".format(format_time_diff(start_time, end_time))
        )
...
class CeceApiClient(object):
    ...
    def get_list(self, logger, uri, recursive=False, total_count=0):
        ...
        logger.debug(
            "  GET {0} retrieved {1}/{2} instances".format(uri, total_count, count)
        )
        logger.debug("    next = {0}".format(next))

        if next and recursive:
            results += self.get_list(
                logger, next, recursive=True, total_count=total_count
            )
