...
def get_or_create_color(logger, p, product, product_ctpk, color_ctpk, client, cmd_name):

    """ Get or create Color. Match on **name** only! """

    color, created = Color.objects.get_or_create(name=p["color"])
    logger.debug("  {0} Color: {1}".format("Created" if created else "Have", color))
...
def get_or_create_categories(
    logger, p, product, product_ctpk, category_ctpk, client, cmd_name
):
    ...
    for c in p["category"]:
        ...
        logger.debug(
            "  {0} Category: {1} ({2})".format(
                "Created" if created else "Have",
                category,
...
def get_or_create_subcategories(
    logger, p, product, product_ctpk, category_ctpk, subcategory_ctpk, client, cmd_name
):
    ...
    for sc in p["subcategory"]:
        ...
        logger.debug(
            "  {0} (parent) Category: {1} ({2})".format(
                "Created" if created else "Have", parent, parent.get_section_display()
            )
        )
...
def get_or_create_material(
    logger, p, product, product_ctpk, material_ctpk, client, cmd_name
):
    ...
    for m in p["material"]:
        ...
        logger.debug(
            "  {0} Material: {1}".format("Created" if created else "Have", material)
        )
...
def get_or_create_size(logger, p, product, product_ctpk, size_ctpk, client, cmd_name):
    ...
    for s in p["size"]:  # NB external has 'size' singular, but is M2M!
        ...
        logger.debug("  {0} Size: {1}".format("Created" if created else "Have", size))
...
def create_or_update_products(logger, cmd_name, client, recursive=True):
    ...
    for i, p in enumerate(data):
        try:
            ...
            product.from_price = (
                Money(p["old_price"], "EUR") if p["old_price"] else None
            )
            ...
            product.save()
            logger.debug(
                "  {0} Product: {1}".format("Created" if created else "Have", product)
            )
            # end of fields

            # Related field: external color is a string (one color per instance)
            get_or_create_color(
                logger, p, product, product_ctpk, color_ctpk, client, cmd_name
            )
            ...
            get_or_create_size(
                logger, p, product, product_ctpk, size_ctpk, client, cmd_name
            )
            ### End of product
            ...
            img_dir = "{0}/img/products/{1}".format(settings.STATIC_ROOT, store.slug)
            ...
            for i, img_url in enumerate([p["primary_image"]] + extra_images):
                ...
                if "cece" not in img_url:
                    ...
                    if response.status_code != 200:
                        logger.error(
                            "    Could not retrieve img_url = '{0}'.".format(img_url)
                        )
                        continue
                    extension = response.headers["Content-Type"].replace("image/", "")
                    fname = "{0}.{1}".format(
                        hashlib.md5(img_url.encode("utf-8")).hexdigest()[0:7], extension
                    )
            ...
            thumbnail_path = "{0}/{1}".format(
                str(settings.STATIC_ROOT), urlparse(thumbnail).path
            ).replace("//static", "")
