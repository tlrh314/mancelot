...
def create_or_update_paymentoptions(logger, cmd_name, client, recursive=True):
    ...
    for i, pm in enumerate(data):
        ...
        save_to = "{0}/img/logos/payment/{1}".format(
            settings.STATIC_ROOT, os.path.basename(fname)
        )
...
class Command(CommandWrapper):
    help = (
        "\033[91mUpdate PaymentOption with Cece data, overwriting all fields!\033[0m\n"
    )
